...
class API_Controller extends REST_Controller
{
	...
	{	
		...
		if ($conds['item_search']==1) {
			...
			if ($conds['is_paid'] == "only_paid_item") {
				...
				if ( !empty( $limit ) && !empty( $offset )) {
					// if limit & offset is not empty
					$data = $this->Item->get_all_item_by_paid_date( $conds, $limit, $offset )->result();


				} else if ( !empty( $limit )) {
					// if limit is not empty
					$data = $this->Item->get_all_item_by_paid_date( $conds, $limit )->result();

				} else {
					// if both are empty
					$data_paid = $this->Item->get_all_item_by_paid_date( $conds )->result();
	...
	*/
	...
	function send_android_fcm2( $registatoin_ids, $message) 
    {
