...
class Images extends API_Controller
{
	...
	function delete_img_post(){
		$img_id = $this->post('img_id');
		if($img_id != ''){
			$this->Image->delete_img($img_id);
			$this->success_response( get_msg('Image deleted successfully.') );
		}
	}
				...
				if(strtolower($path_parts['extension']) != "jpeg" && strtolower($path_parts['extension']) != "png" && strtolower($path_parts['extension']) != "jpg") {
					...
					if (move_uploaded_file($_FILES['file']['tmp_name'], $uploaddir . $filename)) {
						...
						if(copy($uploaddir . $filename,$uploaddir_thumb . $filename)){
							...
							//add watermark
					        //$watermark_path = $path_parts;
					        //$this->add_watermark($path_parts);
						    $item_img_data = array( 
	...
	function add_watermark($path_parts){

		$imgConfig = array();
												
		$imgConfig['image_library']   = 'GD2';
								
		$imgConfig['source_image']    = $path_parts;
								
		$imgConfig['wm_type']       = 'overlay'; 

		$imgConfig['wm_overlay_path'] = './uploads/risee-watermark-Copy.png';
		
		//$imgConfig['wm_opacity'] = 100; 

		$imgConfig['wm_hor_alignment'] = 'right'; 

		$imgConfig['wm_padding'] = '-5';
	...
	function upload_lrs_post()
	{
		
		$lrs_id = $this->post('lrs_id');
		$files = $this->post('file');
		$img_id = $this->post('img_id');
	...
	function upload_clickearn_post()
	{
		
		$item_id = $this->post('click_earn_id');
		$files = $this->post('file');
		$img_id = $this->post('img_id');
	...
	function banner_by_admin_post()
	{
		...
		if($device_token == $check_device_token){
			...
			$item_id = $this->post('bnr_parent_id');
			$files = $this->post('file');
			$img_id = $this->post('img_id');
	 ...
	 * all_flats
	 */
	function all_flats_get() 
	{
		$project_id = $this->get( 'project_id' );

		$noti_obj = $this->Project_flats->get_flats_data($project_id);
 
		$this->custom_response( $noti_obj );
