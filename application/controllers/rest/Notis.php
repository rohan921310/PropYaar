...
class Notis extends API_Controller
{
	...
	*/
	...
	function usr_noti_post()
	{

		$user_name = $this->post('user_name');
		$mobile_number = $this->post('mobile_number');
		$property_id =$this->post('property_id');
		$property_title = $this->Pending->get_one($property_id)->title;

		$message = "Hi, RISEE user(".$user_name.",".$mobile_number.") seen your contact details for your property(" . $property_title .").";
		...
		if($user_device_token != "") {
			...
			if($status->success === 1 && $status->failure === 0){
			     $this->success_response( get_msg( 'Sent' ));
			}else{
