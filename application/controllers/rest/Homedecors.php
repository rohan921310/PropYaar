 ...
 * REST API for News
 */
class Homedecors extends API_Controller
{
	...
	{
		parent::__construct( 'Homedecors_model' );
	}
	...
	function add_post() {
		...
		if($id != ""){
		    $response = $this->Homedecors_model->save($agents_data,$id);
		    if($response){
			...
			$response = $this->Homedecors_model->save($agents_data);
			if($response){
	...
	function homedecors_delete_post( ){
		...
		if ( !$this->Homedecors_model->delete_by( $conds_id )) {
