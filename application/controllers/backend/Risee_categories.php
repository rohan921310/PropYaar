 ...
 */
class Risee_categories extends BE_Controller {

	/**
	 * Constructs required variables
	 */
	function __construct() {
		parent::__construct( MODULE_CONTROL, 'risee_categories_module' );
	}
	...
	function index() {
		...
		// get rows count
		$this->data['rows_count'] = $this->Risee_category->count_all_by( $conds );

		$this->data['risee_category'] = $this->Risee_category->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
	...
	function edit( $id ) {
		...
		// load user
		$this->data['category'] = $this->Risee_category->get_one( $id );
	...
	function save( $id = false ) {
		...
		// save category
		if ( ! $this->Risee_category->save( $data, $id )) {
	...
	function delete( $category_id ) {
		...
		if ( !$this->ps_delete->delete_risee_category($category_id )) {
	...
	{		
		 ...
		 $conds['group_name'] = $name;
		 	...
		 	if( $cat_id != "") {
		 		// echo "bbbb";die;
				if ( strtolower( $this->Risee_category->get_one( $id )->group_name ) == strtolower( $name )) {
				// if the name is existing name for that user id,
					return true;
				} 
			} else {
				// echo "aaaa";die;
				if ( $this->Risee_category->exists( ($conds ))) {
	...
	function ajx_publish( $category_id = 0 )
	{
		...
		// save data
		if ( $this->Risee_category->save( $category_data, $category_id )) {
			echo 'true';
		} else {
			echo 'false';
		}
