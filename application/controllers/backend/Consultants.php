 ...
 */
class Consultants extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'consultants_module' );
	}
	...
	function index() {
		...
		// get rows count
		$this->data['rows_count'] = $this->Consultant_model->count_all_by( $conds );
		
		// get conditions
		$this->data['consultants'] = $this->Consultant_model->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
		// load index logic
		parent::index();
	}
	...
	function add() {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'Add Consultants' );
	...
	function edit( $id ) {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'Edit Consultants' );

		// load user
		$this->data['consultants'] = $this->Consultant_model->get_one( $id );
	...
	function save( $id = false ) {
		...
		// save category
		if ( ! $this->Consultant_model->save( $data, $id )) {
	...
	function delete( $id ) {
		...
		if ( !$this->ps_delete->delete_consultants( $id )) {
	...
	{		
		 ...
		 $conds['name'] = $name;
		 	...
		 	if( $id != "") {
				if ( strtolower( $this->Consultant_model->get_one( $id )->name ) == strtolower( $name )) {
				// if the name is existing name for that user id,
					return true;
				} else if ( $this->Consultant_model->exists( ($conds ))) {
					// if the name is existed in the system,
					$this->form_validation->set_message('is_valid_name', get_msg( 'err_dup_name' ));
					return false;
				}
			} else {
				if ( $this->Consultant_model->exists( ($conds ))) {
				// if the name is existed in the system,
					$this->form_validation->set_message('is_valid_name', get_msg( 'err_dup_name' ));
					return false;
				}
