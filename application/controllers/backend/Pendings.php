...
class Pendings extends BE_Controller {
	...
	function save( $id = false ) {
		...
		$item_title = $this->Item->get_one( $id )->title;

		//// Start - Send Noti /////
		if($data['status'] == 1) {
			//approve so change status to publish (1)
			$title = 'Approved';
			$message = get_msg( 'approve_message_1' ) . $item_title . get_msg( 'approve_message_2' );
		} else if ($data['status'] == 2) {
			//disable so change status to publish (2)
			$title = 'Disabled';
			$message = "Your Posted Item(". $item_title .") has been Disabled. ";
		} else if ($data['status'] == 3){
			//reject so change status to reject (3)
			$title = 'Rejected';
			$message = "Your Posted Item(". $item_title .") has been Rejected.";    //get_msg( 'reject_message_1' ) . $data['title'] . get_msg( 'reject_message_2' );
		}
		$error_msg = "";
		$success_device_log = "";
