 ...
 */
class Realty_share_property extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'realty_share_property_module' );
	}
	...
	function index() {
		...
		// get rows count
		$this->data['rows_count'] = $this->Realty_share_properties->count_all_by( $conds );
		
		// get conditions
		$this->data['realty_share_properties'] = $this->Realty_share_properties->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
		// load index logic
		parent::index();
	}
	...
	function add() {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'Add Realty Share Properties' );
	...
	function edit( $id ) {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'Edit Realty Share Properties' );

		// load user
		$this->data['realty_share_properties'] = $this->Realty_share_properties->get_one( $id );
	...
	function save( $id = false ) {
		...
		// save category
		if ( ! $this->Realty_share_properties->save( $data, $id )) {
	...
	function delete( $id ) {
		...
		if ( !$this->ps_delete->delete_realty_share_property( $id )) {
	...
	function get_item_id()
	{
		if ( $this->input->post('property_title')) {
			...
			$itemid = $this->Realty_share_properties->item_id($title);
	...
	{		
		 ...
		 $conds['property_name'] = $name;
		 	...
		 	if( $id != "") {
				if ( strtolower( $this->Realty_share_properties->get_one( $id )->property_name ) == strtolower( $name )) {
				// if the name is existing name for that user id,
					return true;
				} else if ( $this->Realty_share_properties->exists( ($conds ))) {
					// if the name is existed in the system,
					$this->form_validation->set_message('is_valid_name', get_msg( 'err_dup_name' ));
					return false;
				}
			} else {
				if ( $this->Realty_share_properties->exists( ($conds ))) {
				// if the name is existed in the system,
					$this->form_validation->set_message('is_valid_name', get_msg( 'err_dup_name' ));
					return false;
				}
