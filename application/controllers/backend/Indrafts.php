 ...
 * Items Controller
 */
class Indrafts extends BE_Controller {
	/**
	 * Construt required variables
	 */
	function __construct() {
		parent::__construct( MODULE_CONTROL, 'indraft_items_module' );
	}
	/**
	...
	function index() {
		...
		// get rows count
        $this->data['rows_count'] = $this->Indraft->count_all_by( $conds );
        // print_r($this->data['rows_count']) ;
		// exit;
		// get categories
		$this->data['indrafts'] = $this->Indraft->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
		// load index logic
		parent::index();
	}
	/**
	...
	function save( $id = false ) {
		...
		$error_msg = "";
		$success_device_log = "";
		$added_user_id = $this->Indraft->get_one($id)->added_user_id;
	...
	*/
	function edit( $id ) 
	{
		...
		// load user
		$this->data['indraft'] = $this->Indraft->get_one( $id );
	...
	function delete( $id ) 
	{
	    ...
	    $enable_trigger = true;
	    if ( ! $this->ps_delete->delete_indraft( $id, $enable_trigger )) {
			// set error message
			$this->set_flash_msg( 'error', get_msg( 'err_model' ));
			// rollback
			$this->trans_rollback();
