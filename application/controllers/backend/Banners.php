...
class Banners extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'banner_module' );
	}
	...
	function index() {
		...
		// get blogs
		$this->data['blogs'] = $this->Banner->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
	...
	function save( $id = false ) {
		...
		// prepare blog url
		//if ( $this->has_data( 'feed_url' )) {
		//	$data['feed_url'] = $this->get_data( 'feed_url' );
		//}
	...
	*/
	function edit( $id ) 
	{
		...
		// load user
		$this->data['blog'] = $this->Banner->get_one( $id );
	...
	function ajx_publish( $feed_id = 0 )
	{
		...
		// save data
		if ( $this->Banner->save( $feed_data, $feed_id )) {
			echo true;
		} else {
			echo false;
		}
