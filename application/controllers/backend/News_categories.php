 ...
 */
class News_categories extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'news_categories_module' );
	}
	...
	function index() {
		...
		// get rows count
		$this->data['rows_count'] = $this->News_category->count_all_by( $conds );
		
		// get categories
		$this->data['categories'] = $this->News_category->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
		// load index logic
		parent::index();
	}
	...
	function edit( $id ) {
		...
		// load user
		$this->data['category'] = $this->News_category->get_one( $id );
	...
	function save( $id = false ) {
		 ...
		 */
		if ( !$id ) {
			if ( ! $this->insert_icon_images( $_FILES, 'news_category', $data['cat_id'], "cover" )) {
			...
			if ( ! $this->insert_icon_images( $_FILES, 'news-category-icon', $data['cat_id'], "icon" )) {
				// if error in saving image
	...
	function delete( $category_id ) {
		...
		if ( !$this->ps_delete->delete_news_category( $category_id )) {
	...
	{		
		 ...
		 $conds['cat_name'] = $name;
		 	...
		 	if( $cat_id != "") {
		 		// echo "bbbb";die;
				if ( strtolower( $this->News_category->get_one( $id )->cat_name ) == strtolower( $name )) {
				// if the name is existing name for that user id,
					return true;
				} 
			} else {
				// echo "aaaa";die;
				if ( $this->News_category->exists( ($conds ))) {
	...
	function ajx_publish( $category_id = 0 )
	{
		...
		// save data
		if ( $this->News_category->save( $category_data, $category_id )) {
			echo 'true';
		} else {
			echo 'false';
		}
