	...
	<?php if ( !empty( $agents ) && count( $agents->result()) > 0 ): ?>
		...
		<?php foreach($agents->result() as $agent): ?>
			...
			<tr>
				...
				<td ><?php echo $agent->rating;?></td>

				<?php if ( $this->ps_auth->has_access( EDIT )): ?>
			
					<td>
						<a href='<?php echo $module_site_url .'/edit/'. $agent->id; ?>'>
				...
				<?php if ( $this->ps_auth->has_access( DEL )): ?>
					...
					<td>
						<a herf='#' class='btn-delete' data-toggle="modal" data-target="#myModal" id="<?php echo $agent->id;?>">
